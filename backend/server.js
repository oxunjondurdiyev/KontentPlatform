...
function syncEnvToDb() {
  try {
    ...
    const keys = ["GROQ_API_KEY","GOOGLE_AI_KEY","TELEGRAM_BOT_TOKEN","TELEGRAM_CHANNEL_ID","ADMIN_TELEGRAM_CHAT_ID","INSTAGRAM_ACCESS_TOKEN","INSTAGRAM_USER_ID","YOUTUBE_API_KEY","YOUTUBE_CLIENT_ID","YOUTUBE_CLIENT_SECRET","YOUTUBE_REFRESH_TOKEN","FACEBOOK_PAGE_ACCESS_TOKEN","FACEBOOK_PAGE_ID","KLING_API_KEY","RUNWAY_API_KEY","VIDEO_PROVIDER"];
    for (const key of keys) { if (process.env[key]) Settings.set(key, process.env[key]); }
    if (process.env.GEMINI_API_KEY && !process.env.GOOGLE_AI_KEY) Settings.set("GOOGLE_AI_KEY", process.env.GEMINI_API_KEY);
  } catch (e) { console.error("Sync xato:", e.message); }
}
...
function createSuperadmin() {
  ...
  } catch (e) { console.error("Superadmin xatosi:", e.message); }
}
...
app.listen(PORT, () => { console.log("KontentBot Pro http://localhost:" + PORT + " da ishga tushdi"); });
module.exports = app;
