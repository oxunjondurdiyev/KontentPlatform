...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Telefon majburiy va raqam tekshiruvi
    const phoneDigits = form.phone.replace(/\D/g, '');
    if (!form.phone.trim()) { setError("Telefon raqam majburiy"); return; }
    if (phoneDigits.length < 9 || phoneDigits.length > 13) { setError(t('auth.invalidPhone')); return; }
    setError('');
    setLoading(true);
    try {
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label={t('auth.phone')}>
                <input type="tel" required className={inputCls} placeholder="+998901234567"
                  value={form.phone} onChange={set('phone')} />
              </Field>
              <Field label={t('auth.passport')}>
                <input type="text" className={inputCls} placeholder="AA1234567"
