    ...
    <div className="p-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        {[
          ...
          { label: 'Faol foydalanuvchi', value: stats.active_users || 0 },
      ...
      <div className="card mb-6">
        ...
        {testResult && (
          <div className="mt-3 space-y-1">
            {Object.entries(testResult).map(([platform, r]) => (
              <div key={platform} className={`text-sm px-3 py-1.5 rounded ${
                r.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
              }`}>
                {r.success ? '✅' : '❌'} {platform}: {r.success ? 'Muvaffaqiyatli!' : r.error}
              </div>
            ))}
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-gray-500 border-b">
                <th className="pb-2">Ism / Email</th>
                <th className="pb-2">Reja</th>
                <th className="pb-2">Status</th>
                <th className="pb-2">Obuna tugaydi</th>
