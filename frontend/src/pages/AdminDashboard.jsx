    ...
    <div className="p-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        {[
          ...
          { label: t('admin.activeUsers'), value: stats.active_users || 0 },
          { label: t('admin.paidUsers'), value: stats.paid_users || 0 },
          { label: t('admin.totalContents'), value: stats.total_contents || 0 },
          { label: t('admin.thisMonth'), value: stats.this_month || 0 }
        ].map(s => (
      ...
      <div className="card">
        <h3 className="font-semibold mb-4 dark:text-gray-100">{t('admin.users')} ({users.length})</h3>
                ...
                <tr key={u.id}>
                  ...
                  <td className="py-2 pr-3">
                    <select value={u.plan} onChange={e => updateUser(u.id, { plan: e.target.value })}
                      className={"text-xs px-2 py-1 rounded-full border-0 font-medium " + (PLAN_COLORS[u.plan] || PLAN_COLORS.free)}>
                      {PLAN_IDS.map(p => <option key={p} value={p}>{t('plan.' + p)}</option>)}
                    </select>
                  </td>
                  <td className="py-2 pr-3">
                    <button onClick={() => updateUser(u.id, { is_active: u.is_active ? 0 : 1 })}
                      className={"text-xs px-2 py-1 rounded-full " + (u.is_active ? "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400" : "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400")}>
